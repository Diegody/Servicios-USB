...
$sql_table = "SELECT MAX(NUMEROSESION)+1 AS SESION FROM ACADEMICO.PROGRAMACION_TUTORIAS_TB WHERE DOCUMENTO = '$doc_est' AND PERIODOACADEMICO = '$ciclo' 
AND PROFESORRESPONSABLE IN (SELECT NOMBREPROFESOR||' '||APELLIDOSPROFESOR NOMBRES FROM ACADEMICO.PROFESORES_TUTORIAS_TB WHERE DOCUMENTOP = '$doc_doc')";
